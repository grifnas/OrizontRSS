...
public static class NewsBlurFeedSnapshotPolicy
{
    ...
    /// <summary>Compares feed addresses conservatively, accepting only the common www alias.</summary>
    public static bool AddressesMatch(string? left, string? right)
    {
        var leftKey = AddressKey(left);
        var rightKey = AddressKey(right);
    ...
    /// <summary>
    /// Finds a unique remote feed for a locally queued deletion. URL equality is
    /// preferred; a unique normalized-name match is the fallback for providers
    /// that publish the same feed through a FeedBurner/canonical URL.
    /// </summary>
    public static NewsBlurFeedInfo? FindRemoteForPendingDeletion(
        NewsBlurPendingFeedDeletion pending,
        IEnumerable<NewsBlurFeedInfo> remoteFeeds)
    {
        var feeds = remoteFeeds.ToList();
        if (!string.IsNullOrWhiteSpace(pending.FeedId))
        {
            var byId = feeds.FirstOrDefault(feed => string.Equals(feed.FeedId, pending.FeedId, StringComparison.OrdinalIgnoreCase));
            if (byId is not null) return byId;
        }

        var byAddress = feeds.Where(feed => AddressesMatch(pending.Url, feed.Url)).ToList();
        if (byAddress.Count == 1) return byAddress[0];

        var pendingName = NormalizeFeedName(pending.Name);
        if (pendingName.Length < 6) return null;
        var byName = feeds.Where(feed =>
        {
            var remoteName = NormalizeFeedName(feed.Name);
            return remoteName.Length >= 6 &&
                (string.Equals(remoteName, pendingName, StringComparison.OrdinalIgnoreCase) ||
                 remoteName.Contains(pendingName, StringComparison.OrdinalIgnoreCase) ||
                 pendingName.Contains(remoteName, StringComparison.OrdinalIgnoreCase));
        }).ToList();
        return byName.Count == 1 ? byName[0] : null;
    }

    private static string RemoveWwwPrefix(string key)
    {
        var separator = key.IndexOf('/');
        var host = separator < 0 ? key : key[..separator];
        var suffix = separator < 0 ? string.Empty : key[separator..];
        return host.StartsWith("www.", StringComparison.OrdinalIgnoreCase)
            ? host[4..] + suffix
            : host + suffix;
    }

    private static string NormalizeFeedName(string? value)
    {
        ...
        return new string(withoutMarks.Where(char.IsLetterOrDigit).ToArray()).ToLowerInvariant();
    }
    ...
    public static bool IsRemoteDeletion(Feed localFeed, IReadOnlyCollection<NewsBlurFeedInfo> remoteFeeds)
    {
        ...
        return !remoteFeeds.Any(remote =>
            ...
            AddressesMatch(remote.Url, localFeed.Url));
    }
...
public static class NewsBlurStartupSyncPolicy
{
    public static bool ShouldSyncFeedsAndFolders(AppSettings settings, bool sessionAvailable) =>
        sessionAvailable && settings.NewsBlurSyncFeedsAndFoldersAtStartup;
...
public sealed class NewsBlurConnection
{
    ...
    public async Task<IReadOnlyList<NewsBlurStory>> GetStoriesAsync(string sessionId, string feedId, DateTimeOffset? oldestLocalArticle = null, int maxPages = 5, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(sessionId)) throw new InvalidOperationException(T("Sesiunea NewsBlur nu este disponibilă."));
    ...
    private static HttpClient CreateClient(HttpClientHandler handler)
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd("OrizontRSS/1.6.0");
